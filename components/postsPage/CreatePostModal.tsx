							...
							<div className="mt-4 flex flex-col gap-3">
								...
								<div className="flex gap-3 items-center">
									...
									<div className="flex gap-2 max-w-[70%]">
										{ selectedImages ? (
											Array.from(selectedImages).map(e => <p title={e.name} key={e.size} className="text-ellipsis overflow-hidden whitespace-nowrap">{ e.name }</p>)
										): null }
									</div>
								</div>
								<div className="flex gap-3 items-center">
									...
									<input
										...
										onChange={(e: any) => setSelectedOtherFiles(e.target.files)}
									/>
									<div className="flex gap-2 max-w-[70%]">
										{ selectedOtherFiles ? (
											Array.from(selectedOtherFiles).map(e => <p title={e.name} key={e.size} className="text-ellipsis overflow-hidden whitespace-nowrap">{ e.name }</p>)
										): null }
									</div>
								</div>
								<div className="w-[30%]">
